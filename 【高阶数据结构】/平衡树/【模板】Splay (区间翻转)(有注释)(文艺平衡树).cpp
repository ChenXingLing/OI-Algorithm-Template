...
inline void pushdown(Re p){//下传懒标记 
...
inline void Splay(Re p,Re to){//旋至目标地点 
...
//把l~r全部移到 根(l-1)和 根的右儿子(r+1)中间，方便后面的操作 
    Splay(l,0),Splay(r,l);//将l-1旋到根，r+1旋到根的右儿子
    tr[tr[r].ps[0]].tag^=1;
}
inline void print(Re p){//递归中序遍历输出答案 
    pushdown(p);
    if(pl)print(pl);
...
int main(){
    in(n),in(T);
    a[1]=-inf,a[n+2]=inf;//插入0和n+1防止越界 
    for(Re i=2;i<=n+1;++i)a[i]=i-1;//插~了个0，所以其他数都要往后推一位 
    build(root,1,n+2,0);//多插入了两个数0和n+1 
    while(T--)in(x),in(y),reverse(x+1,y+1);//因为插入了0，次序要加一 
    print(root);
}
