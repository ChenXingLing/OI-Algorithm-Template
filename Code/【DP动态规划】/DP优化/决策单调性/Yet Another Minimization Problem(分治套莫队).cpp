...
struct MODUI{
	int nowL,nowR,cnt[N];LL ans;
	inline LL calc(Re L,Re R){
		while(nowR<R)++nowR,ans+=cnt[A[nowR]]++;
		while(nowL>L)--nowL,ans+=cnt[A[nowL]]++;
		while(nowR>R)ans-=--cnt[A[nowR]],--nowR;
		while(nowL<L)ans-=--cnt[A[nowL]],++nowL;
		return ans;
	}
}TR;
