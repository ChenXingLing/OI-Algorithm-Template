...
using namespace std;
const int N=260,M=1540,inf=2e9;//µãÊý: 16*16+2=258, ±ßÊý: 16*16*(4+2)=1536
...
int main(){
	...
	while(Q1--)in(z),in(x),in(y),add_(st,Poi(x+1,y+1),z,0);
	while(Q2--)in(z),in(x),in(y),add_(Poi(x+1,y+1),ed,z,0);
	EK(st,ed);
	printf("%lld",maxcost);
}
